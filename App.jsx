...
// ─── LIVE FLOOR PAGE ──────────────────────────────────────────────────────────
function LiveFloorPage({ employees, schedule, shifts, attendance, setAttendance }) {
  const [now, setNow] = useState(new Date());
  const [breaks, setBreaks] = useState({}); // empId → { start, durationMin, extraBreaks:[] }
  const [showBreakModal, setShowBreakModal] = useState(null); // empId
  const [extraStart, setExtraStart]   = useState("");
  const [extraDur, setExtraDur]       = useState(15);

  // Live clock
  useState(()=>{ const t=setInterval(()=>setNow(new Date()),60000); return()=>clearInterval(t); });
  useRef(()=>{ const t=setInterval(()=>setNow(new Date()),60000); return ()=>clearInterval(t); });
  ...
  const workingNow = employees.filter(emp => {
    ...
    return nowMin>=st || nowMin<en; // midnight-crossing
  });

  function getBreak(empId) { return breaks[empId]||{start:null,durationMin:30,extraBreaks:[]}; }

  function setBreakStart(empId) {
    const t = pad(now.getHours())+":"+pad(now.getMinutes());
    setBreaks(prev=>({...prev,[empId]:{...getBreak(empId),start:t}}));
  }
  function endBreak(empId) {
    setBreaks(prev=>({...prev,[empId]:{...getBreak(empId),start:null}}));
  }
  function addExtraBreak(empId) {
    ...
    setExtraStart(""); setExtraDur(15); setShowBreakModal(null);
  }

  // Per-employee status
  function getEmpStatus(emp) {
    const b = getBreak(emp.id);
    const att = ((attendance[todayD]||{})[emp.id])||{};
    if (b.start) {
      const breakStartMin = toMin(b.start);
      const elapsed = nowMin >= breakStartMin ? nowMin-breakStartMin : nowMin+1440-breakStartMin;
      const over = elapsed - (b.durationMin||30);
      return { status:"Break", elapsed, over, isOvertime: over>0 };
    }
    return { status:"Online", elapsed:0, over:0, isOvertime:false };
  }

  // Total extra break time per employee
  function totalExtraMin(empId) {
    return (getBreak(empId).extraBreaks||[]).reduce((s,b)=>s+b.durationMin,0);
  }

  // Pressure indicator
  const onlineCount = workingNow.filter(e=>getEmpStatus(e).status==="Online").length;
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:12 }}>
        {workingNow.map(emp => {
          ...
          const statusColor = es.status==="Online"?"#10B981":"#F59E0B";
          const borderColor = es.isOvertime?"#EF4444":statusColor;

          return (
            <div key={emp.id} style={{ ...CRD({ padding:16 }), border:`2px solid ${borderColor}40`,
              ...
              {/* Status duration bar */}
              {es.status==="Break" && b.durationMin && (
                <div style={{ marginBottom:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"#64748B", marginBottom:3 }}>
                    <span>Break Progress</span><span>{es.elapsed}/{b.durationMin}m</span>
                  </div>
                  <div style={{ background:"#F1F5F9", borderRadius:10, height:8, overflow:"hidden" }}>
                    <div style={{ width:Math.min(100,Math.round(es.elapsed/b.durationMin*100))+"%", height:"100%",
                      background: es.isOvertime?"#EF4444":"#F59E0B", borderRadius:10, transition:"width 1s" }}/>
                  </div>
                </div>
              )}
...
function ReportsPage({ employees, schedule, shifts, attendance, performance, heatmap, kg, queueLog }) {
  ...
  const [selectedShifts, setSelectedShifts] = useState([]); // [] = all shifts
  ...
  function getShiftEmps(dayName) {
    return employees.filter(emp => {
      ...
      if (selectedShifts.length === 0) return true; // all
  ...
  function buildDailyReport() {
    const dayName = DAYS[new Date(date+"T12:00:00").getDay()];
    const dayEmps = getShiftEmps(dayName);
    ...
    return `📊 DAILY OPERATIONS REPORT — ${date} [${shiftLabel}]
${"=".repeat(55)}
...
${perfSorted.map((e,i)=>{
  ...
  const dur = p.workDuration ? `${Math.floor(p.workDuration/60)}h${p.workDuration%60}m` : "—";
  return `${["🥇","🥈","🥉"][i]||`${i+1}.`} ${e.name}: ${p.closed||0} closed | Esc: ${p.escalations||0} | Work: ${dur}`;
}).join("\n")}
    ...
    <div>
      ...
      {reportType!=="scorecard" && (
        <div style={{ fontFamily:"monospace", fontSize:12, whiteSpace:"pre-wrap", lineHeight:1.7,
          maxHeight:600, overflow:"auto", background:"#0F172A", borderRadius:10, padding:20, color:"#E2E8F0" }}>
          {getReportText()}
        </div>
      )}
    </div>
...
function AuditLogPage({ auditLog, session }) {
  ...
  const [activityFilter, setActivityFilter] = useState("All"); // "All" | "Online" | "Offline"
...
function LoginScreen({ onLogin, employees }) {
  ...
  function tryLogin() {
    if (isAgent) {
      // Agent: no password
      onLogin({ role: selectedRole, name: "Agent" });
      return;
    }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0F2744 0%,#1E3A5F 50%,#0F2744 100%)",
      ...
      <div style={{ width:460, background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 24px 80px rgba(0,0,0,0.4)" }}>
        ...
        <div style={{ padding:"24px 32px 28px" }}>
              ...
              <button key={role} onClick={()=>{ setSelectedRole(role); setSelectedName(""); setError(""); setPassword(""); setStep("login"); }}
                ...
                <div>
                  ...
                  <div style={{ fontSize:10, color:"#94A3B8" }}>
                    {role==="Agent" ? "No password · View only" : "Personal password"}
                  </div>
                </div>
              </button>
            ))}
          ...
          {!isAgent && selectedName && step==="setup" && (
            <div style={{ background:"#EFF6FF", border:"1px solid #BFDBFE", borderRadius:8, padding:"14px", marginBottom:14 }}>
              <div style={{ fontWeight:700, color:"#1D4ED8", fontSize:13, marginBottom:10 }}>
