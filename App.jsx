...
function LiveFloorPage({ employees, schedule, shifts, attendance, setAttendance }) {
  ...
  const [breaks, setBreaks] = useState({}); // empId → { start, end, durationMin, extraBreaks:[] }
  const [showBreakModal, setShowBreakModal] = useState(null); // empId — for scheduling break
  const [showExtraModal, setShowExtraModal] = useState(null); // empId — for extra break
  const [breakStart, setBreakStartTime] = useState("");
  const [breakEnd, setBreakEndTime]     = useState("");
